...
class ReportDAO extends BaseDAO
{
    ...
    {
        try {
            $query = $this->db->createQuery("SELECT r, ue, ( 6371 * ACOS( COS( RADIANS(:lat) ) * COS( RADIANS( r.latitude ) ) * COS( RADIANS( r.longitude ) - RADIANS(:long) ) + SIN( RADIANS(:lat) ) * SIN( RADIANS( r.latitude ) ) ) ) AS distance FROM App\Model\Report AS r JOIN r.user AS ue HAVING distance < 3 ORDER BY distance");
